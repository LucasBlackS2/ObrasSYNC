...
export default function GerenciarMateriais() {
  ...
  const [area, setArea] = useState('12'); // exemplo fixo de área
  const [quantidadeTotal, setQuantidadeTotal] = useState('');
  const [valorTotal, setValorTotal] = useState('');
  const [materiais, setMateriais] = useState<Material[]>([]);

  const calcularQuantidade = () => {
    const qtd = parseFloat(area) * parseFloat(quantidadeM2 || '0');
    setQuantidadeTotal(qtd.toFixed(2));
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.tip}>💡 Dica: Calcular por m²</Text>
      <TextInput
        style={styles.input}
        placeholder="Ex: 1"
        keyboardType="numeric"
        value={quantidadeM2}
        onChangeText={setQuantidadeM2}
      />
      <TouchableOpacity style={styles.calcButton} onPress={calcularQuantidade}>
        <Text style={styles.buttonText}>Calcular</Text>
      </TouchableOpacity>
      <Text style={styles.calcText}>
        {area}m² × {quantidadeM2 || 0} = {quantidadeTotal || 0} {unidade}
      </Text>
      ...
      {quantidadeTotal && valorTotal ? (
        <Text style={styles.tip}>
          Preço unitário: R$ {(parseFloat(valorTotal) / parseFloat(quantidadeTotal)).toFixed(2)} por {unidade}
        </Text>
      ) : null}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#dfd4d4', padding: 16 },
  ...
  input: { backgroundColor: '#3f3c3c', padding: 10, borderRadius: 8, marginVertical: 5 },
  ...
  calcText: { fontSize: 14, color: '#333', marginVertical: 5 },
  ...
  materialItem: { backgroundColor: '#fff', padding: 10, borderRadius: 8, marginVertical: 5 },
  materialText: { fontSize: 14, color: '#333' },
});